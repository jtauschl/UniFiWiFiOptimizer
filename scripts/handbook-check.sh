...
SW_DEV_HANDBOOK_DOC_REF="${SW_DEV_HANDBOOK_DOC_REF:-v0.10.1}"
...
# 15. no_int_content_leak — the code repo must never contain content that belongs in
# <project>-int, unconditionally (../../02-bootstrap/project-setup.md#documentation-companion-repo)
# — never gated on whether the code repo happens to be public or private, since visibility is a
# GitHub setting that can change while a piece of content's own classification does not. This check
# only catches the single highest-signal leak pattern — a literal reference to the companion repo's
# own name/path (e.g. a pasted internal path, an ADR excerpt naming its source) — appearing in the
# code repo's own TRACKED files outside the one expected, already-mandated cross-reference in
# README.md (project-setup.md's own "each repo's README.md links to the other"). It does NOT catch
# a leak that never mentions <project>-int by name (see "Known gaps" below) — narrowed in scope
# rather than downgraded in severity, same convention as stack_folder_shape's own best-effort
# framing above. FAIL, waivable (not in NON_WAIVABLE_CHECKS): a project with a legitimately
# different, still-safe cross-reference shape (e.g. a full URL instead of the bare string) can
# waive with a documented reason via handbook-baseline.yml's exceptions[].
check_no_int_content_leak() {
  if [ -z "$INT_DIR" ] || [ ! -d "$INT_DIR" ]; then
    record_outcome no_int_content_leak PASS "not applicable — no companion repo to check against (see int_present)"
    return
  fi
  local needle="${PROJECT_NAME}-int"
  local leaks
  leaks="$(git -C "$CODE_REPO_DIR" grep -I -l -F -- "$needle" -- . ':!README.md' 2>/dev/null || true)"
  if [ -n "$leaks" ]; then
    record_outcome no_int_content_leak FAIL "code repo contains a reference to '$needle' outside README.md, in: $(printf '%s' "$leaks" | tr '\n' ' ') — see ../../02-bootstrap/project-setup.md#documentation-companion-repo (this check only lists the offending file paths, never the matching line content, so it can't itself become a second leak vector)"
    return
  fi
  if git -C "$CODE_REPO_DIR" grep -q -F -- "$needle" -- README.md 2>/dev/null; then
    record_outcome no_int_content_leak PASS "no '$needle' reference found outside README.md's own expected cross-link"
  else
    record_outcome no_int_content_leak FAIL "README.md does not link to $needle — see ../../02-bootstrap/project-setup.md#documentation-companion-repo ('each repo's README.md links to the other')"
  fi
}
