...
# exist and its availability tracks the same plan gate — a 403 on it signals one of three distinct
# causes, each handled separately below: the product isn't on this plan, the repo is a fork (GitHub
# restricts Dependency Review on forks specifically), or the token/app lacks Contents: read on this
# repo (a narrower-scoped credential can read basic repo metadata while still lacking this) — the
# same way the secret-scanning field-presence check above signals its own gate.
#
