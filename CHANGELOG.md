...
### Fixed
- The "Below corridor before TX adjustment" (°) note no longer hedges with "may or may not bring this neighbor back into the corridor" — by the time a neighbor is marked `°` rather than escalated to the unfixable `*` Coverage gap warning, the corridor formula (or the capped-projection check) has already established that the recommended TX change resolves it. The note now states that plainly instead of expressing false uncertainty.
