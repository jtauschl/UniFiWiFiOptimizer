...
- `Throughput`/`Latency` profiles: `sae_anti_clogging` corrected from 10 to 5 — the value was never live-verified (unlike the rest of those profiles' fields) and Ubiquiti's own guidance is to leave SAE Anti-clogging/Sync Time at the controller default, which is 5 (already used by the other three profiles).
